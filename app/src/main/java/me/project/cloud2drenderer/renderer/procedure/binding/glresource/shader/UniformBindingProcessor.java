...
public class UniformBindingProcessor {
    ...
    private static ShaderVariableSetterWrapper getUniformVarSetterWrapper(Field field, Object object, @NonNull ShaderVariableMeta meta){
        ...
        if (meta.intBased) {
            ...
            if(isArray) {
                ...
                UniformVar<Integer> intUniformVar;
                try {
                    Object val = field.get(object);
                    assert val instanceof UniformVar;
                    intUniformVar = (UniformVar<Integer>) val;
                } catch (IllegalAccessException e) {
                    throw new RuntimeException(e);
                }
                // TODO: value类型转换在被调用时才发生，需要有一种方法将类型转换移动到调用之前
                setterWrapper = () -> intScalarSetter.set(intUniformVar.getLocation(), intUniformVar.getValue());
                uniformVar = intUniformVar;
            }
        }else {
    ...
    private static void processFields(@NonNull RenderContext context, @NonNull List<Field> fields, Map<String, ShaderVariableMeta> uniformMetas, Object object, String fieldNamePrefix, String uniformNamePrefix){
        for (Field field : fields) {
            ...
            String currentAnnoUniformName = anno.uniformName();
            String uniformName = uniformNamePrefix + currentAnnoUniformName;
            ...
            if((mask&UniformFlag.IS_STRUCT.mask)!=0){
                try {
                    String newFieldNamePrefix = fieldName;
                    String newUniformNamePrefix = uniformName;
                    if(!(currentAnnoUniformName.isEmpty()||currentAnnoUniformName.isBlank())){
                        newUniformNamePrefix += ".";
                    }
                    generateShaderUniformSetters(context,uniformMetas, Objects.requireNonNull(field.get(object)),
                            newFieldNamePrefix,newUniformNamePrefix);
                    continue;
                } catch (IllegalAccessException e) {
            ...
            if((mask&UniformFlag.USING_RAW.mask)!=0) {
                setterWrapper = getRawFieldSetterWrapper(field, object,meta);
            }else {
                setterWrapper = getUniformVarSetterWrapper(field, object, meta);
                try {
